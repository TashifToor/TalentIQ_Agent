...
function AnalysisCarousel({ text }: { text: string }) {
  ...
  if (steps.length === 0) return (
    <div style={{ fontSize: 12, color: 'var(--dash-text-muted)', padding: '12px 0' }}>No analysis available.</div>
  )
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: 14, flexWrap: 'wrap' }}>
        {steps.map((s, i) => (
          <button key={i} onClick={() => setActive(i)} style={{
            ...
            border: `1px solid ${active === i ? STEP_COLORS[i % STEP_COLORS.length] : 'var(--dash-border-soft)'}`,
            background: active === i ? `${STEP_COLORS[i % STEP_COLORS.length]}18` : 'transparent',
            color: active === i ? STEP_COLORS[i % STEP_COLORS.length] : 'var(--dash-text-muted)',
            transition: 'all .2s',
          }}>
      ...
      {/* Active step */}
      <div style={{
        padding: '16px 18px', background: 'var(--dash-surface-2)', borderRadius: 10,
        borderLeft: `3px solid ${STEP_COLORS[active % STEP_COLORS.length]}`,
        transition: 'all .25s',
      }}>
        <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 8, color: 'var(--dash-text)' }}>
          {STEP_ICONS[active % STEP_ICONS.length]} {steps[active]?.heading}
        </div>
        <p style={{ fontSize: 12.5, color: 'var(--dash-text-muted)', lineHeight: 1.8, margin: 0, whiteSpace: 'pre-wrap' }}>
          {steps[active]?.body}
        </p>
      </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'center', gap: 6, marginTop: 12 }}>
        {steps.map((_, i) => (
          <div key={i} onClick={() => setActive(i)} style={{
            ...
            background: active === i ? STEP_COLORS[i % STEP_COLORS.length] : 'var(--dash-overlay-09)',
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--dash-bg)', fontFamily: 'Inter, sans-serif', color: 'var(--dash-text)' }}>
      <style jsx global>{`
        ...
        .history-row:hover { background: var(--dash-surface-2); }
      ...
      <div style={{ maxWidth: 760, margin: '0 auto', padding: '40px 24px 80px' }}>
        ...
        <div style={{ fontSize: 13, color: 'var(--dash-text-muted)', marginBottom: 28 }}>All your past CV screenings — click any scan to see the full breakdown</div>
        ...
        {loading ? (
          <div style={{ fontSize: 13, color: 'var(--dash-text-muted)', textAlign: 'center', padding: '40px 0' }}>Loading…</div>
        ) : history.length === 0 ? (
          <div style={{ background: 'var(--dash-surface)', border: '1px solid var(--dash-border-soft)', borderRadius: 12, padding: 40, textAlign: 'center' }}>
            <div style={{ fontSize: 13, color: 'var(--dash-text-muted)', marginBottom: 14 }}>No scans yet.</div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {history.map((h, i) => {
              ...
              return (
                <div key={h.id ?? i} className="fade-up" style={{ background: 'var(--dash-surface)', border: '1px solid var(--dash-border-soft)', borderRadius: 10, overflow: 'hidden', animationDelay: `${i * 40}ms` }}>
                  <div className="history-row" onClick={() => setExpandedId(isOpen ? null : (h.id ?? i))} style={{ padding: '16px 20px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                      ...
                      <svg className="chevron" style={{ transform: isOpen ? 'rotate(90deg)' : 'rotate(0deg)', flexShrink: 0 }} width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--dash-text-muted)" strokeWidth="2"><path d="M9 18l6-6-6-6" /></svg>
                    </div>
                  </div>

                  {isOpen && (
                    <div className="expand-in" style={{ borderTop: '1px solid var(--dash-border-soft)', padding: '20px 20px 22px', background: 'var(--dash-surface-2)' }}>
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 18 }}>
                        <div>
                          ...
                          {(h.matched_skills?.length ?? 0) === 0 ? (
                            <div style={{ fontSize: 12, color: 'var(--dash-text-faint)' }}>None recorded.</div>
                          ) : (
                        ...
                        <div>
                          ...
                          {(h.missing_skills?.length ?? 0) === 0 ? (
                            <div style={{ fontSize: 12, color: 'var(--dash-text-faint)' }}>None — full match.</div>
                          ) : (
                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                      ...
                      <div style={{ fontSize: 11, fontWeight: 600, color: 'var(--dash-text-muted)', letterSpacing: '.05em', textTransform: 'uppercase', marginBottom: 12 }}>Full Analysis</div>
