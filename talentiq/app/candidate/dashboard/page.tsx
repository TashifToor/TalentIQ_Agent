...
import { api, ApiError, clearAuthState } from '@/lib/api'
...
const ICONS: Record<string, JSX.Element> = {
  ...
  activity: <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><rect x="3" y="4" width="18" height="17" rx="2"/><path d="M3 9h18M8 2v4M16 2v4"/></svg>,
}
...
export default function CandidateDashboard() {
  ...
  const S = { sidebar: { width: 220, flexShrink: 0, background: 'var(--dash-sidebar)', borderRight: '1px solid var(--dash-sidebar-border)', display: 'flex', flexDirection: 'column' as const } }

  return (
    <NotificationProvider>
    <div className="app-shell" style={{ display: 'flex', height: '100vh', overflow: 'hidden', background: 'var(--dash-bg)', fontFamily: 'Inter, sans-serif', color: 'var(--dash-text)' }}>
      <style jsx global>{`
        ...
        .nav-btn:hover { background: var(--dash-overlay-035); color: var(--dash-text); }
        .history-row { transition: background .2s, transform .2s; }
        .history-row:hover { background: var(--dash-surface-2); transform: translateX(2px); }
      ...
      {/* SIDEBAR */}
      <div style={S.sidebar}>
        <Link href="/" style={{ padding: '22px 18px', borderBottom: '1px solid var(--dash-sidebar-border)', display: 'flex', alignItems: 'center', gap: 10, fontFamily: "Inter, sans-serif", fontSize: 20, fontWeight: 600, color: 'var(--dash-text)', textDecoration: 'none' }}>
        ...
        <div style={{ padding: '18px 12px 6px', fontSize: 10, fontWeight: 700, letterSpacing: '.12em', textTransform: 'uppercase', color: 'var(--dash-text-faint)' }}>Menu</div>
        {NAV.map(n => {
          const active = activeNav === n.key
          return (
            <Link key={n.key} href={n.href} onClick={() => handleNavClick(n)} className="nav-btn" style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 14px', borderRadius: 8, fontSize: 13, fontWeight: 500, color: active ? '#e2b04a' : 'var(--dash-text-muted)', cursor: 'pointer', margin: '0 6px 2px', border: active ? '1px solid rgba(226,176,74,.15)' : '1px solid transparent', background: active ? 'rgba(226,176,74,.1)' : 'transparent', fontFamily: 'Inter, sans-serif', width: 'calc(100% - 12px)', textAlign: 'left', textDecoration: 'none' }}>
              <span style={{ display: 'flex' }}>{ICONS[n.key]}</span>{n.label}
            </Link>
          )
        })}
        <div style={{ marginTop: 'auto', padding: '14px', borderTop: '1px solid var(--dash-sidebar-border)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 10px', borderRadius: 8 }}>
            ...
            <div style={{ minWidth: 0, flex: 1 }}>
              <div style={{ fontSize: 13, fontWeight: 600, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', color: 'var(--dash-text)' }}>{user?.name || 'Loading…'}</div>
              <div style={{ fontSize: 11, color: 'var(--dash-text-faint)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{user?.email || ''}</div>
            </div>
            <button
              onClick={() => {
                clearAuthState()
              ...
              title="Log out"
              style={{ background: 'none', border: 'none', color: 'var(--dash-text-muted)', cursor: 'pointer', flexShrink: 0, display: 'flex', padding: 4 }}
            >
      ...
      {/* MAIN */}
      <div className="app-main" style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', background: 'var(--dash-bg)' }}>
        {/* Topbar */}
        <div style={{ height: 60, borderBottom: '1px solid var(--dash-border)', display: 'flex', alignItems: 'center', padding: '0 28px', justifyContent: 'space-between', flexShrink: 0 }}>
        ...
        <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          {/* Scan Area */}
          <div id="scan-area" style={{ background: 'var(--dash-surface)', border: '1px solid var(--dash-border-soft)', borderRadius: 12, padding: 24 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
              <div><div style={{ fontSize: 14, fontWeight: 600, letterSpacing: '-.2px' }}>Scan Your CV</div><div style={{ fontSize: 12, color: 'var(--dash-text-muted)', marginTop: 2 }}>Upload a CV and paste a job description to get your match score</div></div>
              <span style={{ fontSize: 12, color: 'var(--dash-text-muted)' }}>{scansLeft} free scan{scansLeft === 1 ? '' : 's'} left</span>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              ...
              <div onClick={handlePickFile} className="dropzone" style={{ border: `2px dashed ${file ? 'rgba(19,194,142,.4)' : 'var(--dash-overlay-14)'}`, borderRadius: 10, padding: 28, textAlign: 'center', cursor: 'pointer', background: file ? 'rgba(19,194,142,.04)' : 'var(--dash-surface-2)' }}>
                <div style={{ marginBottom: 10, display: 'flex', justifyContent: 'center' }}>
                  {uploading ? (
                    ...
                    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="var(--dash-text-muted)" strokeWidth="1.6"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><path d="M14 2v6h6"/></svg>
                  )}
                </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                <div style={{ fontSize: 11, fontWeight: 600, color: 'var(--dash-text-muted)', letterSpacing: '.06em', textTransform: 'uppercase' }}>Job Description</div>
                <textarea value={jd} onChange={e => setJd(e.target.value)} placeholder={"Paste the job description here…\n\ne.g. 'We're looking for a Senior React Developer with 4+ years…'"} style={{ flex: 1, background: 'var(--dash-surface-2)', border: '1px solid var(--dash-border-soft)', borderRadius: 10, padding: 14, fontSize: 13, fontFamily: 'Inter, sans-serif', color: 'var(--dash-text)', outline: 'none', resize: 'none', lineHeight: 1.6, minHeight: 100 }} />
                  ...
                  <div style={{ marginTop: 12 }}>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                      {SCAN_STEPS.map((step, i) => (
                        <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, opacity: i <= scanStep ? 1 : 0.2, transition: 'opacity 0.4s ease', fontSize: 11, color: i === scanStep ? '#e2b04a' : i < scanStep ? '#13c28e' : 'var(--dash-text-muted)' }}>
                          <div style={{ width: 5, height: 5, borderRadius: '50%', background: i === scanStep ? '#e2b04a' : i < scanStep ? '#13c28e' : 'var(--dash-overlay-09)', flexShrink: 0, transition: 'background 0.4s' }} />
                    ...
                    <div style={{ marginTop: 10, height: 2, background: 'var(--dash-overlay-05)', borderRadius: 1 }}>
          ...
          {result && (
            <div className="fade-up" style={{ display: 'grid', gridTemplateColumns: 'auto 1fr', gap: 20, alignItems: 'start' }}>
              {/* Score Ring */}
              <div style={{ background: 'var(--dash-surface)', border: '1px solid var(--dash-border-soft)', borderRadius: 12, padding: 24, display: 'flex', flexDirection: 'column', alignItems: 'center', minWidth: 200 }}>
                ...
                <div style={{ position: 'relative', width: 120, height: 120, marginBottom: 16 }}>
                  ...
                  <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
                    ...
                    <small style={{ fontSize: 11, color: 'var(--dash-text-muted)', marginTop: 2 }}>/ 100</small>
                  </div>
                </div>
                ...
                <div style={{ fontSize: 12, color: 'var(--dash-text-muted)', textAlign: 'center', lineHeight: 1.5, marginBottom: 14 }}>You match {result.metrics?.candidate_score ?? 0}% of<br />this role's requirements</div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6, width: '100%' }}>
                  ...
                  {result.flags?.has_min_experience === false && (
                    <div style={{ fontSize: 11, color: 'var(--dash-text-muted)', background: 'var(--dash-surface-2)', border: '1px solid var(--dash-border-soft)', borderRadius: 6, padding: '5px 10px', textAlign: 'center' }}>Below Minimum Experience</div>
                  )}
                </div>
              </div>

              {/* Analysis Panel */}
              <div style={{ background: 'var(--dash-surface)', border: '1px solid var(--dash-border-soft)', borderRadius: 12, padding: 24 }}>
                <div style={{ display: 'flex', gap: 0, borderBottom: '1px solid var(--dash-border-soft)', marginBottom: 20 }}>
                  {['skills', 'gaps', 'recos'].map(t => (
                    <button key={t} onClick={() => setActiveTab(t)} className="tab-btn" style={{ padding: '8px 16px', fontSize: 13, fontWeight: 500, color: activeTab === t ? '#e2b04a' : 'var(--dash-text-muted)', cursor: 'pointer', borderBottom: `2px solid ${activeTab === t ? '#e2b04a' : 'transparent'}`, marginBottom: -1, border: 'none', borderTop: 'none', borderLeft: 'none', borderRight: 'none', background: 'none', fontFamily: 'Inter, sans-serif' }}>
                ...
                {activeTab === 'skills' && (
                  (result.metrics?.matched_skills?.length ?? 0) === 0 ? (
                    <div style={{ fontSize: 12, color: 'var(--dash-text-muted)', textAlign: 'center', padding: '20px 0' }}>No matched skills found.</div>
                  ) : (result.metrics?.matched_skills || []).map((s: string, i: number) => (
                    <div key={s} className="fade-up" style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', background: 'var(--dash-surface-2)', borderRadius: 8, border: '1px solid var(--dash-border-soft)', marginBottom: 8, animationDelay: `${i * 50}ms` }}>
                      ...
                      <span style={{ fontSize: 13, color: 'var(--dash-text)' }}>{s}</span>
                    </div>
                  ))
                )}
                ...
                {activeTab === 'gaps' && (
                  (result.metrics?.missing_skills?.length ?? 0) === 0 ? (
                    <div style={{ fontSize: 12, color: 'var(--dash-text-muted)', textAlign: 'center', padding: '20px 0' }}>No skill gaps found — strong match.</div>
                    ...
                    <div key={s} className="fade-up" style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', background: 'var(--dash-surface-2)', borderRadius: 8, border: '1px solid var(--dash-border-soft)', marginBottom: 8, animationDelay: `${i * 50}ms` }}>
                      ...
                      <span style={{ fontSize: 13, color: 'var(--dash-text)', flex: 1 }}>{s}</span>
                ...
                {activeTab === 'recos' && (
                  <div className="fade-up" style={{ fontSize: 13, color: 'var(--dash-text)', lineHeight: 1.8, whiteSpace: 'pre-wrap' }}>
                    {(result.deep_analysis || 'No suggestions available.')
                      ...
                      .map((block: string, i: number) => {
                        ...
                        return (
                          <div key={i} style={{ marginBottom: 16, padding: '12px 14px', background: 'var(--dash-surface-2)', borderRadius: 8, borderLeft: '3px solid #e2b04a' }}>
                            {heading && <h5 style={{ fontSize: 13, fontWeight: 600, marginBottom: 6, color: 'var(--dash-text)' }}>{heading}</h5>}
                            <p style={{ fontSize: 12, color: 'var(--dash-text-muted)', lineHeight: 1.7, margin: 0 }}>{body.replace(/^\n+/, '')}</p>
                          </div>
                        )
                      })}
                  </div>
          ...
          {/* History */}
          <div id="history" style={{ background: 'var(--dash-surface)', border: '1px solid var(--dash-border-soft)', borderRadius: 12, padding: 24 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
              <div style={{ fontSize: 13, fontWeight: 600 }}>Recent Scans</div>
              {history.length > 0 && (
                <button onClick={() => setHistory([])} style={{ fontSize: 11, color: 'var(--dash-text-muted)', background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'Inter, sans-serif' }}>Clear</button>
              )}
            </div>
            {history.length === 0 ? (
              <div style={{ fontSize: 12, color: 'var(--dash-text-muted)', textAlign: 'center', padding: '20px 0' }}>No scans yet — run your first analysis above.</div>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {history.map((h, i) => (
                  <div key={i} className="history-row fade-up" style={{ display: 'flex', alignItems: 'center', gap: 14, background: 'var(--dash-surface-2)', border: '1px solid var(--dash-border-soft)', borderRadius: 10, padding: '14px 18px', cursor: 'pointer', animationDelay: `${i * 50}ms` }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ fontSize: 12, color: 'var(--dash-text-muted)' }}>{h.skills}</div>
                    </div>
                    <div style={{ fontSize: 11, color: 'var(--dash-text-muted)' }}>{h.date}</div>
                    <div style={{ color: 'var(--dash-text-muted)', fontSize: 16 }}>›</div>
                  </div>
                ))}
              </div>
            )}
