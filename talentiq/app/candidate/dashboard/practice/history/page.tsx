...
export default function PracticeHistoryPage() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f7f5f0', fontFamily: 'Inter, sans-serif', color: '#1f1c17' }}>
      <div style={{ height: 56, borderBottom: '1px solid #e7e4da', display: 'flex', alignItems: 'center', padding: '0 28px', gap: 16 }}>
        <Link href="/candidate/dashboard/practice" style={{ color: '#7a7468', textDecoration: 'none', fontSize: 13 }}>← Interview Practice</Link>
        <span style={{ color: '#c9c4b6' }}>·</span>
        <span style={{ fontSize: 13, color: '#3a352d', fontWeight: 600 }}>Practice History</span>
      </div>
      ...
      <div style={{ maxWidth: 720, margin: '0 auto', padding: '40px 24px' }}>
        ...
        {items === null && !error && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            <LoadingSkeleton height={64} light /><LoadingSkeleton height={64} light /><LoadingSkeleton height={64} light />
          </div>
        )}

        {items && items.length === 0 && (
          <GlassCard light>
            <EmptyState icon="🎯" title="No practice sessions yet" description="Start one from Interview Practice — chat, assessment, or voice." light />
          </GlassCard>
        )}
        ...
        {items && items.length > 0 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {items.map(item => {
              ...
              return (
                <div key={item.id}>
                  <GlassCard light hoverable onClick={() => openItem(item.id)} style={{ padding: '14px 16px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontSize: 13.5, fontWeight: 700, color: '#1f1c17' }}>{item.target_role}</div>
                        <div style={{ fontSize: 11, color: '#7a7468' }}>{m.title} · {item.created_at ? new Date(item.created_at).toLocaleDateString() : ''}</div>
                      </div>
                      {score != null && (
                        <div style={{ fontSize: 15, fontWeight: 700, color: score >= 70 ? '#13c28e' : score >= 45 ? '#e2b04a' : '#ef4444' }}>{score}</div>
                      )}
                      <GradientBadge label={item.status === 'completed' ? 'Done' : 'In progress'} tone={item.status === 'completed' ? 'teal' : 'neutral'} light />
                      <button onClick={e => { e.stopPropagation(); deleteItem(item.id) }} disabled={deletingId === item.id}
                        style={{ background: 'none', border: 'none', color: '#9c9689', cursor: 'pointer', fontSize: 13, padding: 4 }}>
                        {deletingId === item.id ? '...' : '✕'}
                      </button>
                    </div>
                  </GlassCard>
