...
export default function HRSignupPage() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError("Workspace provisioning failed or license key invalid.");
    } finally { setLoading(false); }
  };

  // Real OAuth Native Pop-up Logic for Recruiters
  const handleGoogleSignIn = async () => {
    setError("");
    const result = await signIn("google", { 
      redirect: false, 
      callbackUrl: "/hr/billing-gate" 
    });
    ...
    if (result?.url) {
      ...
      const checkPopup = setInterval(() => {
        if (!popup || popup.closed) {
          ...
          // Recruiter automatically goes to HR dashboard workspace route
          router.push("/hr/billing-gate");
        }
      }, 1000);
    } else {
      setError("Failed to initialize enterprise authentication pipeline.");
    }
  };

  return (
    <div className="min-h-screen bg-[#0b0f19] text-slate-200 flex overflow-hidden">
      {/* Left Marketing Panel for HR/Enterprise (Clean Blue Accent Theme) */}
      <div className="hidden lg:flex flex-col justify-between w-[45%] bg-slate-950/20 p-12 border-r border-slate-900 relative">
        ...
        <div className="space-y-6 relative z-10">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-500/5 border border-blue-500/20 text-[10px] font-mono text-blue-400">
            <Zap size={10} /> Multi-Tenant B2B Portal
          </div>
          <h1 className="text-4xl font-extrabold text-white leading-tight">
            Scale Recruitment via <br />
            <span className="bg-gradient-to-r from-blue-400 to-indigo-400 bg-clip-text text-transparent">Agentic RAG</span>
          </h1>
          <p className="text-slate-400 text-sm max-w-sm">
            Deploy stateful LangGraph agents to run parallel mass resume screening operations, structured grading, and background matching logs.
          </p>

          <div className="space-y-3 pt-2">
            {[
              "Bulk batch uploading & metadata extractions",
              "Stateful cross-verification workflows",
              "Custom organizational scoring metrics",
              "Role-based multi-tenant user permissions"
            ].map((text, i) => (
        ...
        <div className="text-xs text-slate-600 font-mono">Enterprise Workspace Tier · Elastic Ingestion Queues</div>
      </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-8">
        <div className="w-full max-w-md bg-slate-950/20 border border-slate-900 rounded-2xl p-8 backdrop-blur-md shadow-2xl">
          <div className="mb-8">
            <h2 className="text-2xl font-bold text-white tracking-tight">Provision HR Workspace</h2>
            <p className="text-slate-400 mt-1 text-xs">Register your corporate email handle to allocate an enterprise environment</p>
          </div>
          ...
          <form onSubmit={handleSignup} className="space-y-4">
            ...
            <div className="space-y-1.5">
              <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Workspace Password</label>
              <div className="relative">
            ...
            <button type="submit" disabled={loading} className="w-full flex items-center justify-center gap-2 py-3 rounded-xl text-xs font-semibold bg-blue-500 hover:bg-blue-600 text-white shadow-lg shadow-blue-500/10 transition-all">
              {loading ? "Provisioning System..." : <><span className="font-bold">Setup Enterprise Workspace</span><ArrowRight size={14} /></>}
            </button>
          </form>
          ...
          {/* Fixed Pop-up Google Button for HR */}
          <button 
            type="button"
          ...
          <p className="text-center text-xs text-slate-500 mt-6">
            ...
            <Link href="/auth/login/hr" className="text-blue-400 hover:text-blue-300 font-semibold transition-colors">Sign In Here</Link>
          </p>
        </div>
      </div>
    </div>
