...
const gold = '#e2b04a'
const panel = 'var(--dash-surface)'
const border = 'var(--dash-border)'
const textDim = 'var(--dash-text-muted)'
    ...
    return (
        <div style={{ position: 'relative', width: size, height: size, flexShrink: 0 }}>
            <svg width={size} height={size} style={{ transform: 'rotate(-90deg)' }}>
                <circle cx={size / 2} cy={size / 2} r={radius} stroke="var(--dash-text-faint)" strokeWidth={7} fill="none" />
...
function MiniBar({ label, score }: { label: string; score: number }) {
    ...
    return (
        <div style={{ marginBottom: 10 }}>
            ...
            <div style={{ height: 6, borderRadius: 4, background: 'var(--dash-overlay-05)', overflow: 'hidden' }}>
        ...
        <div style={{ maxWidth: 1100, margin: '48px auto 0', padding: '0 16px 60px', fontFamily: 'Inter, sans-serif', color: textMain }}>
                ...
                {ats && (
                    <div>
                        ...
                        {ats.top_issues.length > 0 && (
                            <div style={{ background: 'var(--dash-surface-2)', border: `1px solid ${border}`, borderRadius: 10, padding: 14 }}>
                                ...
                                {ats.recommendations.map((r, i) => (
                                    <div key={i} style={{ fontSize: 12.5, color: 'var(--dash-text)', marginBottom: 6, lineHeight: 1.5 }}>• {r}</div>
                                ))}
                            </div>
                        )}
                    </div>
            ...
            <div style={{ ...card, marginBottom: 20 }}>
                ...
                <textarea value={jd} onChange={e => setJd(e.target.value)} placeholder="Paste the target job description here…"
                    style={{ width: '100%', minHeight: 100, background: 'var(--dash-surface-2)', border: `1px solid ${border}`, borderRadius: 10, padding: 12, fontSize: 13, color: textMain, outline: 'none', resize: 'vertical', fontFamily: 'inherit', marginBottom: 12, boxSizing: 'border-box' }} />
                ...
                {jobMatch && (
                    <div style={{ marginTop: 20 }}>
                        {beforeAfter && (
                            <div style={{ display: 'flex', gap: 24, alignItems: 'center', background: 'var(--dash-surface-2)', border: `1px solid ${border}`, borderRadius: 10, padding: 16, marginBottom: 16 }}>
                ...
                <div style={{ ...card, marginBottom: 20 }}>
                    ...
                    <ol style={{ margin: 0, paddingLeft: 20 }}>
                        {planItems.map((item, i) => (
                            <li key={i} style={{ fontSize: 13, color: 'var(--dash-text)', marginBottom: 8, lineHeight: 1.6 }}>{item}</li>
                        ))}
                    </ol>
                </div>
            )}
                ...
                {assistantLog.length > 0 && (
                    <div style={{ marginBottom: 20 }}>
                        {assistantLog.map((entry, i) => (
                            <div key={i} style={{ marginBottom: 12, background: 'var(--dash-surface-2)', border: `1px solid ${border}`, borderRadius: 10, padding: 12 }}>
                                <div style={{ fontSize: 12, fontWeight: 700, color: gold, marginBottom: 4 }}>{entry.question}</div>
                                <div style={{ fontSize: 12.5, color: 'var(--dash-text)', lineHeight: 1.6 }}>{entry.answer}</div>
                            </div>
                        ))}
                    </div>
                )}
                ...
                {resumeSource === 'builder' && activeCv.experience.length > 0 && (
                    <div style={{ borderTop: `1px solid ${border}`, paddingTop: 16 }}>
                        ...
                        <select
                            ...
                            style={{ width: '100%', background: 'var(--dash-surface-2)', border: `1px solid ${border}`, borderRadius: 8, padding: '9px 12px', fontSize: 12.5, color: textMain, marginBottom: 10 }}
                        >
                        ...
                        {rewriteResult && (
                            <div style={{ background: 'var(--dash-surface-2)', border: `1px solid ${border}`, borderRadius: 10, padding: 12 }}>
                                <div style={{ fontSize: 11, color: textDim, marginBottom: 6 }}>Original</div>
                                <div style={{ fontSize: 12.5, color: 'var(--dash-text-muted)', marginBottom: 10, lineHeight: 1.5 }}>{rewriteResult.original}</div>
