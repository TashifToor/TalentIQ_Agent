...
export default function DecisionCenter({ candidate, onClose, onDecided }: {
    ...
    const labelSt = getLabelSt(light)
        ...
        <div role="dialog" aria-modal="true" aria-label="Decision Center" style={{
            ...
            <GlassCard className="modal-sheet" style={{ width: 520, maxWidth: '100%', maxHeight: '90vh', overflowY: 'auto', padding: 0 }}>
                <div style={{ padding: '18px 22px', borderBottom: `1px solid ${light ? 'var(--dash-border-soft)' : 'rgba(255,255,255,.06)'}`, display: 'flex', alignItems: 'center' }}>
                    <div style={{ fontSize: 15, fontWeight: 700, color: (light ? 'var(--dash-text)' : '#fff'), flex: 1 }}>Decision Center</div>
                ...
                <div style={{ padding: 22 }}>
                    {alreadyDecided && step === 'error' && !decision && (
                        <div>
                            <div style={{ fontSize: 13, color: (light ? 'var(--dash-text-muted)' : 'rgba(255,255,255,.6)'), marginBottom: 14 }}>
                    ...
                    {step === 'choose' && (
                        <div>
                            <div style={{ fontSize: 12, color: (light ? 'var(--dash-text-muted)' : 'rgba(255,255,255,.4)'), marginBottom: 16 }}>
                            ...
                            <div className="responsive-grid-2" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                                <button onClick={() => choose('accepted')} style={decisionCardSt('#13c28e', light)}>
                                    <div style={{ fontSize: 18, marginBottom: 6 }}>✓</div>
                                    <div style={{ fontSize: 13, fontWeight: 700, color: (light ? 'var(--dash-text)' : '#fff'), marginBottom: 3 }}>Accept Candidate</div>
                                    <div style={{ fontSize: 11, color: (light ? 'var(--dash-text-muted)' : 'rgba(255,255,255,.4)') }}>Move this candidate forward</div>
                                </button>
                                <button onClick={() => choose('rejected')} style={decisionCardSt('#ef4444', light)}>
                                    <div style={{ fontSize: 18, marginBottom: 6 }}>✕</div>
                                    <div style={{ fontSize: 13, fontWeight: 700, color: (light ? 'var(--dash-text)' : '#fff'), marginBottom: 3 }}>Reject Candidate</div>
                                    <div style={{ fontSize: 11, color: (light ? 'var(--dash-text-muted)' : 'rgba(255,255,255,.4)') }}>Close this application</div>
                                </button>
                            </div>
                        </div>
                    )}
                    ...
                    {(step === 'review' || step === 'editing' || step === 'sending') && preview && (
                        <div>
                            ...
                            <div style={{ fontSize: 10, fontWeight: 700, color: (light ? 'var(--dash-text-muted)' : 'rgba(255,255,255,.4)'), textTransform: 'uppercase', letterSpacing: '.05em', marginBottom: 8 }}>Review &amp; Send</div>
                            <div style={{ fontSize: 11, color: (light ? 'var(--dash-text-muted)' : 'rgba(255,255,255,.4)'), marginBottom: 2 }}>To: {candidate.candidate_email || 'No email on file'}</div>
                            ...
                            {step === 'editing' ? (
                                ...
                                <div style={{ background: panelBg, border: `1px solid ${light ? 'var(--dash-border-soft)' : 'rgba(255,255,255,.08)'}`, borderRadius: 10, padding: 14, marginTop: 8 }}>
                                    <div style={{ fontSize: 12.5, fontWeight: 700, color: (light ? 'var(--dash-text)' : '#fff'), marginBottom: 8 }}>{subject}</div>
                                    <div style={{ fontSize: 12, color: (light ? 'var(--dash-text-muted)' : 'rgba(255,255,255,.6)'), whiteSpace: 'pre-wrap', lineHeight: 1.6 }}>{body}</div>
                                </div>
                            )}

                            <label style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 14, fontSize: 12, color: (light ? 'var(--dash-text-muted)' : 'rgba(255,255,255,.6)'), cursor: 'pointer' }}>
                    ...
                    {step === 'done' && result && (
                        <div style={{ textAlign: 'center', padding: '20px 0' }}>
                            ...
                            <div style={{ fontSize: 14, fontWeight: 700, color: (light ? 'var(--dash-text)' : '#fff'), marginBottom: 6 }}>
...
const getLabelSt = (light: boolean): React.CSSProperties => ({ display: 'block', fontSize: 10.5, fontWeight: 700, color: light ? 'var(--dash-text-muted)' : 'rgba(255,255,255,.4)', margin: '10px 0 4px' })
const getInputSt = (light: boolean): React.CSSProperties => ({ width: '100%', background: light ? 'var(--dash-surface)' : '#161614', border: `1px solid ${light ? 'var(--dash-border-soft)' : 'rgba(255,255,255,.1)'}`, borderRadius: 8, padding: '9px 11px', fontSize: 12.5, color: light ? 'var(--dash-text)' : '#fff', outline: 'none', boxSizing: 'border-box' })
...
function decisionCardSt(accent: string, light: boolean): React.CSSProperties {
    return {
        textAlign: 'left', padding: 16, borderRadius: 12, borderLeft: `3px solid ${accent}`,
        border: `1px solid ${light ? 'var(--dash-border-soft)' : 'rgba(255,255,255,.08)'}`, borderLeftWidth: 3, borderLeftColor: accent,
        background: light ? 'var(--dash-surface)' : '#161614', cursor: 'pointer', transition: 'border-color .15s, background .15s',
    }
}