...
export default function FinalReviewStep({
  ...
  return (
    <div style={{ maxWidth: 560 }}>
      <GlassCard style={{ marginBottom: 16, padding: 0, overflow: 'hidden' }}>
        ...
        <div style={{ padding: 20 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
            ...
            <div>
              <div style={{ fontSize: 15, fontWeight: 700, color: '#fff' }}>{data.title || 'Untitled Role'}</div>
              <div style={{ fontSize: 11.5, color: 'rgba(255,255,255,.4)' }}>{data.company || 'No company set'} · {mode.title}</div>
            </div>
          </div>
      ...
      <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12.5, cursor: 'pointer', marginBottom: 18, color: 'rgba(255,255,255,.7)' }}>
