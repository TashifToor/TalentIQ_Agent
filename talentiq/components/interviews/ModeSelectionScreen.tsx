...
import { GlassCard, GradientBadge, FeatureCard, AnimatedButton, FloatingAction } from './shared/primitives'
import { MODE_DEFINITIONS, InterviewMode } from './shared/modeData'
import CopilotPanel from './copilot/CopilotPanel'

export default function ModeSelectionScreen({ onSelect }: { onSelect: (mode: InterviewMode) => void }) {
    ...
    <div className="builder-with-copilot">
      <div style={{ flex: 1 }}>
        <div style={{ marginBottom: 28, textAlign: 'center' }}>
          <GradientBadge label="AI Hiring Operating System" tone="gold" icon="✦" />
          ...
          <div style={{ fontSize: 13, color: 'rgba(255,255,255,.4)' }}>
            Every mode is its own product — pick the one that fits this role.
          </div>
        </div>
                ...
                <GlassCard hoverable glow style={{
                  ...
                  <div style={{ padding: '20px 18px', flex: 1, display: 'flex', flexDirection: 'column' }}>
                    ...
                    <div style={{ marginTop: 'auto', display: 'flex', flexDirection: 'column', gap: 8, paddingTop: 8 }}>
                      ...
                      <AnimatedButton fullWidth onClick={() => onSelect(m.id)}>
                        Create Interview →
                      </AnimatedButton>
                    </div>
                  </div>
      ...
      <CopilotPanel context="Pick a mode on the left — I'll help you configure it once I'm live." />
    </div>
  )
}