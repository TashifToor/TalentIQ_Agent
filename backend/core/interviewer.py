...
def _system_context(job_description: str, extra_questions: list[str]) -> str:
    extra_block = ""
    if extra_questions:
    ...
    return f"""You are conducting a live text-based screening interview on behalf of the hiring company, in the 2026 hiring market where only genuinely exceptional candidates should clear the bar. You are professional, sharp, and courteous — but not a pushover. This is a real screening interview that determines whether this candidate gets a human interview slot, not a friendly chat.
...
{{"message": "<your next question/follow-up, OR a brief closing thank-you if concluding>", "action": "continue" or "conclude"}}"""
...
def generate_report(job_description: str, extra_questions: list[str], transcript: list[dict]) -> dict:
    """
...
- This transcript is a live conversational interview, not a resume — judge substance, specificity, and depth of the candidate's actual answers, not politeness or fluency alone. Vague, generic, or rehearsed-sounding answers with no concrete detail are a red flag, not a pass.
...
Conduct a structured analysis, then score it. Respond ONLY with valid JSON, no markdown, no backticks:

{{
    "experience_assessment": "<2-4 sentences: does the candidate's answers demonstrate the level of hands-on experience the JD requires? Be explicit about years/depth vs what the JD demands — this is a hard constraint, strong communication cannot substitute for a real experience gap.>",
    "deep_analysis": "<4-6 sentences: sharp, specific, unsentimental breakdown of strengths and weaknesses actually evidenced in the transcript — reference specific answers, don't generalize. Call out any vague/deflected answers explicitly.>",
